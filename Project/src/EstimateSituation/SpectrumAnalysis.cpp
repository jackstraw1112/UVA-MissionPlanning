...
// 生成频谱数据
void SpectrumChart::generateSpectrumData()
{
    // 重新生成测试数据前先清空容器，避免重复追加
    m_spectrumData.clear();

    // 频段定义（调整到13GHz范围）
    QVector<SpectrumBand> bands;
    ...
    // 统计各频段的信号数量
    countSignalsInBands(bands);
    
    // 将统计后的频段数据添加到频谱数据中
    m_spectrumData = bands;
}

// 统计各频段的信号数量
void SpectrumChart::countSignalsInBands(QVector<SpectrumBand> &bands)
{
    // 统计雷达信号
    for (const auto &radar : m_radarSource) {
        QString frequency = radar.frequency;
        updateBandSignalCount(bands, frequency);
    }
    
    // 统计通信电台信号
    for (const auto &radio : m_radioSource) {
        QString frequency = radio.frequency;
        updateBandSignalCount(bands, frequency);
    }
    
    // 统计雷达干扰信号
    for (const auto &jammer : m_radarJammerSource) {
        QString frequency = jammer.workingBand;
        updateBandSignalCount(bands, frequency);
    }
    
    // 统计通信干扰信号
    for (const auto &jammer : m_radioJammerSource) {
        QString frequency = jammer.coverageBand;
        updateBandSignalCount(bands, frequency);
    }
}

// 更新频段的信号数量
void SpectrumChart::updateBandSignalCount(QVector<SpectrumBand> &bands, const QString &frequencyStr)
{
    // 解析频率范围
    QString freqStr = frequencyStr;
    // 处理波段名称（如 "S/C波段"）
    if (freqStr.contains("波段")) {
        if (freqStr.contains("S/C")) {
            // S/C波段对应 2-8GHz
            for (auto &band : bands) {
                if (band.name == "S" || band.name == "C") {
                    band.signalCount++;
                }
            }
        }
    ...
    // 处理频率范围（如 "5.2~6.1GHz"）
    QStringList parts = freqStr.split("~");
    if (parts.size() == 2) {
        ...
        // 解析起始频率
        if (startStr.contains("GHz")) {
            startFreq = startStr.replace("GHz", "").toDouble() * 1000; // 转换为 MHz
        } else if (startStr.contains("MHz")) {
            startFreq = startStr.replace("MHz", "").toDouble();
        } else {
            startFreq = startStr.toDouble();
        }
        
        // 解析结束频率
        if (endStr.contains("GHz")) {
            endFreq = endStr.replace("GHz", "").toDouble() * 1000; // 转换为 MHz
        } else if (endStr.contains("MHz")) {
            endFreq = endStr.replace("MHz", "").toDouble();
        } else {
            endFreq = endStr.toDouble();
        }
        
        // 统计该频率范围覆盖的频段
        for (auto &band : bands) {
            QString range = band.range;
            QStringList bandParts = range.split("-");
            if (bandParts.size() == 2) {
                QString bandStartStr = bandParts[0].trimmed();
                QString bandEndStr = bandParts[1].trimmed();
                
                double bandStart = 0.0;
                double bandEnd = 0.0;
                
                if (bandStartStr.contains("GHz")) {
                    bandStart = bandStartStr.replace("GHz", "").toDouble() * 1000;
                } else if (bandStartStr.contains("MHz")) {
                    bandStart = bandStartStr.replace("MHz", "").toDouble();
                } else {
                    bandStart = bandStartStr.toDouble();
                }
                
                if (bandEndStr.contains("GHz")) {
                    bandEnd = bandEndStr.replace("GHz", "").toDouble() * 1000;
                } else if (bandEndStr.contains("MHz")) {
                    bandEnd = bandEndStr.replace("MHz", "").toDouble();
                } else {
                    bandEnd = bandEndStr.toDouble();
                }
                
                // 检查频率范围是否与频段重叠
                if ((startFreq >= bandStart && startFreq < bandEnd) || 
                    (endFreq > bandStart && endFreq <= bandEnd) || 
                    (startFreq <= bandStart && endFreq >= bandEnd)) {
                    band.signalCount++;
                }
            }
        }
...
<<<<<<< HEAD
    ...
    QVector<double> bandRanges;
    m_totalRange = 0.0;

    for (const auto &band : m_spectrumData) {
        QString range = band.range;
        // 解析频率范围
        QStringList parts = range.split("-");
        if (parts.size() == 2) {
            QString startStr = parts[0].replace("GHz", "").replace("MHz", "").trimmed();
            QString endStr = parts[1].replace("GHz", "").replace("MHz", "").trimmed();
            
            double start = startStr.toDouble();
            double end = endStr.toDouble();
            
            // 转换为 MHz
            if (range.contains("GHz")) {
                start *= 1000;
                end *= 1000;
            }
            
            double bandRange = end - start;
            bandRanges.append(bandRange);
            m_totalRange += bandRange;
        } else {
            // 如果无法解析，使用默认值
            bandRanges.append(1.0);
            m_totalRange += 1.0;
        }
    }
    ...
    // 根据频率范围分配宽度
    float x = 50;
    float availableWidth = sceneWidth - 100;

    for (int i = 0; i < barCount; ++i) {
        SpectrumBand &band = m_spectrumData[i];

        // 计算当前频段的宽度比例
        float widthRatio = bandRanges[i] / m_totalRange;
        float barWidth = availableWidth * widthRatio;
